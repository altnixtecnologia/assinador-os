...
export async function updateDocumentLink(docId, link) {
    const { error } = await supabase.from('documentos').update({ link_assinatura: link }).eq('id', docId);
    if (error) throw error;
}
...
export async function checkIfSigned(docId) {
    const { data, error } = await supabase.from('assinaturas').select('id').eq('documento_id', docId);
    if (error) { console.error("Erro em checkIfSigned:", error); return false; }
...
export async function submitSignature(signatureData) {
    const { data, error } = await supabase.functions.invoke('salvar-assinatura', { body: signatureData });
    if (error) throw error;
    return data;
}
